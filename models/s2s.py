...
class conv_block_sp(nn.Module):
    def __init__(self, ch_in, ch_out, down=False, act_fun='LeakyReLU', pad='reflection', group=1, bn_mode="bn",
                 bias=True):
...
class s2s_(nn.Module):
    def __init__(self, img_ch=3, output_ch=3, act_type="LeakyReLU", pad='reflection', bn_mode="bn"):
        ...
        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.concat = Concat_layer(1)
        self.Conv_first1 = conv_block_last(img_ch, enc_ch[0], act_fun=act_type, pad=pad, bn_mode=bn_mode,
                                           bias=False)  # concat
        self.Conv1 = conv_block_sp(ch_in=img_ch + enc_ch[0], ch_out=enc_ch[0], down=True, act_fun=act_type, pad=pad,
                                   bn_mode=bn_mode, bias=False)  # h/2,  w/2
        self.Conv2 = conv_block_sp(ch_in=enc_ch[0], ch_out=enc_ch[1], down=True, act_fun=act_type, pad=pad,
                                   bn_mode=bn_mode, bias=False)  # h/4,  w/4
        self.Conv3 = conv_block_sp(ch_in=enc_ch[1], ch_out=enc_ch[2], down=True, act_fun=act_type, pad=pad,
                                   bn_mode=bn_mode, bias=False)  # h/8,  w/8
        self.Conv4 = conv_block_sp(ch_in=enc_ch[2], ch_out=enc_ch[3], down=True, act_fun=act_type,
        ...
        self.Up_conv1 = conv_block_concat(ch_in=dec_ch[1] + enc_ch[0], ch_out=dec_ch[0], act_fun=act_type, pad=pad,
                                          bn_mode=bn_mode, bias=False)
        self.Conv_last1 = conv_block_last(dec_ch[0] + img_ch, output_ch, act_fun=act_type, pad=pad, bn_mode=bn_mode,
                                          bias=False)  # concat
...
class s2s_fixed(nn.Module):
    def __init__(self, img_ch=3, output_ch=3, act_type="LeakyReLU", pad='reflection', bn_mode="bn"):
        ...
        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.concat = Concat_layer(1)
        self.Conv1 = conv_block_sp(ch_in=img_ch, ch_out=enc_ch[0], down=True, act_fun=act_type, pad=pad,
        ...
        self.Up_conv1 = conv_block_concat(ch_in=dec_ch[1] + enc_ch[0], ch_out=dec_ch[0], act_fun=act_type, pad=pad,
                                          bn_mode=bn_mode)
        self.Conv_last1 = conv_block_last(dec_ch[0] + img_ch, output_ch, act_fun=act_type, pad=pad,
...
class s2s_g(nn.Module):
    ...
    def group_concat(self, d, x, stride1=6, stride2=6):
        return self.concat([d[:, :stride1, :, :], x[:, :stride2, :, :], d[:, stride1:2 * stride1, :, :],
                            x[:, stride2:2 * stride2, :, :],
                            d[:, 2 * stride1:3 * stride1, :, :], x[:, 2 * stride2:3 * stride2, :, :],
                            d[:, 3 * stride1:4 * stride1, :, :], x[:, 3 * stride2:4 * stride2, :, :]])

    def group_mean(self, out, stride=3):
        return (out[:, :stride] + out[:, stride:2 * stride] + out[:, 2 * stride:3 * stride] + out[:,
                                                                                              3 * stride:4 * stride]) / 4.
...
class S2SATnet1(nn.Module):
    def __init__(self, img_ch=3, output_ch=3, act_type="LeakyReLU", bn_mode="bn", pad='reflection'):
        ...
        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.concat = Concat_layer(1)
        self.Conv1 = conv_block_sp(ch_in=img_ch, ch_out=enc_ch[0], down=True, act_fun=act_type, bn_mode=bn_mode,
                                   pad=pad)  # h/2,  w/2
        self.Conv2 = conv_block_sp(ch_in=enc_ch[0], ch_out=enc_ch[1], down=True, act_fun=act_type, bn_mode=bn_mode,
                                   pad=pad)  # h/4,  w/4
        self.Conv3 = conv_block_sp(ch_in=enc_ch[1], ch_out=enc_ch[2], down=True, act_fun=act_type, bn_mode=bn_mode,
                                   pad=pad)  # h/8,  w/8
        self.Conv4 = conv_block_sp(ch_in=enc_ch[2], ch_out=enc_ch[3], down=True, act_fun=act_type, bn_mode=bn_mode,
                                   pad=pad)  # h/16, w/16
        self.Conv5 = conv_block_sp(ch_in=enc_ch[3], ch_out=enc_ch[4], down=True, act_fun=act_type, bn_mode=bn_mode,
                                   pad=pad)  # h/32, w/32
        ...
        self.Up_conv4 = conv_block_concat(ch_in=dec_ch[4] + enc_ch[3], ch_out=dec_ch[3], act_fun=act_type,
                                          bn_mode=bn_mode, pad=pad)
        self.Up_conv3 = conv_block_concat(ch_in=dec_ch[3] + enc_ch[2], ch_out=dec_ch[2], act_fun=act_type,
                                          bn_mode=bn_mode, pad=pad)
        self.Up_conv2 = conv_block_concat(ch_in=dec_ch[2] + enc_ch[1], ch_out=dec_ch[1], act_fun=act_type,
                                          bn_mode=bn_mode, pad=pad)
        self.Up_conv1 = conv_block_concat(ch_in=dec_ch[1] + enc_ch[0], ch_out=dec_ch[0], act_fun=act_type,
                                          bn_mode=bn_mode, pad=pad)
        self.Conv_last1 = conv_block_last(dec_ch[0] + img_ch, output_ch, act_fun=act_type, bn_mode=bn_mode,
                                          pad=pad)  # concat
        self.Sig = nn.Sigmoid()
...
class Testnet(nn.Module):
    def __init__(self, img_ch=3, output_ch=3, act_type="LeakyReLU", pad='reflection'):
        ...
        self.Up_conv4 = conv_block_concat(ch_in=dec_ch[4] + enc_ch[3] + img_ch, ch_out=dec_ch[3], act_fun=act_type,
                                          pad=pad)
        self.Up_conv3 = conv_block_concat(ch_in=dec_ch[3] + enc_ch[2] + img_ch, ch_out=dec_ch[2], act_fun=act_type,
                                          pad=pad)
        self.Up_conv2 = conv_block_concat(ch_in=dec_ch[2] + enc_ch[1] + img_ch, ch_out=dec_ch[1], act_fun=act_type,
                                          pad=pad)
        self.Up_conv1 = conv_block_concat(ch_in=dec_ch[1] + enc_ch[0] + img_ch, ch_out=dec_ch[0], act_fun=act_type,
                                          pad=pad)
        self.Conv_last1 = conv_block_last(dec_ch[0] + img_ch, output_ch, act_fun=act_type, pad=pad)  # concat
        self.Sig = nn.Sigmoid()
